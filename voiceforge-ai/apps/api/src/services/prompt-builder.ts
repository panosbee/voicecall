...
export function buildEnhancedInstructions(params: BuildEnhancedInstructionsParams): string {
  ...
  const calendarInstructions = customerLocale === 'el'
    ? [
        ...
        'Έχεις πρόσβαση σε τρία εργαλεία ημερολογίου:',
        '1. "check_availability" — Ελέγχει ποια ραντεβού είναι διαθέσιμα σε μια ημερομηνία. Επιστρέφει ΠΡΑΓΜΑΤΙΚΑ δεδομένα από το ημερολόγιο.',
        '2. "book_appointment" — Κλείνει ραντεβού σε συγκεκριμένη ημερομηνία και ώρα.',
        '3. "get_business_hours" — Επιστρέφει τις ώρες λειτουργίας, διάρκεια slot, κλειστές ημέρες. Κάλεσέ το αν χρειάζεσαι πληροφορίες ωραρίου.',
        '',
        'ΚΡΙΣΙΜΟΙ ΚΑΝΟΝΕΣ:',
        ...
        '- Αν ο πελάτης ζητήσει συγκεκριμένη ώρα, ΠΡΩΤΑ κάλεσε check_availability για εκείνη την ημέρα. Αν η ώρα δεν είναι στα available_slots, πρότεινε τις κοντινότερες διαθέσιμες.',
        '- Αν κλείσεις ραντεβού και πάρεις απάντηση slot_taken=true, ενημέρωσε τον πελάτη ότι η ώρα είναι πιασμένη και πρότεινε τις εναλλακτικές ώρες.',
        '- Αν πάρεις outside_business_hours=true, ενημέρωσε ότι η ώρα είναι εκτός ωραρίου και πρότεινε αυτή που σου δίνεται.',
        '- Το ωράριο και τα διαθέσιμα slots μπορεί να διαφέρουν ανά ημέρα. Μην υποθέτεις σταθερό ωράριο — βασίζου πάντα στα αποτελέσματα των εργαλείων.',
        '- Αν η μέρα είναι ΠΛΗΡΕΣ κρατημένη, θα λάβεις next_available_days με εναλλακτικές ημερομηνίες — πρότεινέ τες στον πελάτη.',
        ...
        '- Αν δεν υπάρχει διαθεσιμότητα εκείνη τη μέρα: "Δυστυχώς η 12η Μαρτίου είναι πλήρης. Βλέπω διαθεσιμότητα στις 13 Μαρτίου στις 09:00, 10:30 και 14:00. Σας βολεύει;"',
    ...
    : [
        ...
        '1. "check_availability" — Checks which appointment slots are available on a given date. Returns REAL calendar data.',
        '2. "book_appointment" — Books an appointment at a specific date and time.',
        '3. "get_business_hours" — Returns business hours, slot duration, and closed dates. Call it if you need schedule information.',
        '',
        'CRITICAL RULES:',
        '- ALWAYS call "check_availability" BEFORE suggesting or booking any appointment. Never book without checking first.',
        '- If the customer requests a specific time, FIRST call check_availability for that day. If the time is not in available_slots, suggest the nearest available ones.',
        '- If you attempt to book and get slot_taken=true, inform the customer the slot is taken and suggest the alternatives provided.',
        '- If you get outside_business_hours=true, inform the caller the time is outside business hours and suggest the nearest valid time.',
        '- Business hours and available slots may vary by day. Do NOT assume fixed hours — always rely on results from the tools.',
        '- If the day is FULLY booked, you will receive next_available_days with alternative dates — suggest them to the caller.',
        ...
        '- If no availability on that day: "I\'m afraid March 12 is fully booked. I can see openings on March 13 at 09:00, 10:30, and 14:00. Would any of those work?"',
